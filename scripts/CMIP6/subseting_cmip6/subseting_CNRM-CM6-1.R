...
lon_res <- 360 / 256 ###### CHANGES FOR EVERY MODEL
lat_res <- 180 / 128
    ...
    if(ssp == "historical") {
      ...
      for (d in 1:length(dates)){
        ...
        for (p in 1:length(guide$lon)){
          ...
          pixel <- array[X,Y, 1:23741] # change every model 60265 or 23741 for pr
            
          for (r in 1:16){ # 16 for pr 40 for all others
              
            remove  <-  (r * 1460)+1
            pixel <- pixel[-remove]
              
          }
        pixel <- tail(pixel,12775)
    ...
    if(ssp == "ssp245"|ssp == "ssp370"|ssp == "ssp585") {
      ...
      for (d in 1:length(dates)){
        ...
        for (p in 1:length(guide$lon)){
          ...
          pixel <- array[X,Y, 1:31411] # change every model
          
          for (r in 1:21){# removing leap years
